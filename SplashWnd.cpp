  ...
  switch (uMsg)
	{
		case WM_PAINT:
		{
			if (pInstance->m_pImage)
			{
				...
				{
					Gdiplus::Font msgFont( L"宋体", 9, Gdiplus::UnitPixel );
					Gdiplus::SolidBrush msgBrush( static_cast<DWORD>(Gdiplus::Color::IndianRed) );
					gdip.DrawString( s2ws(pInstance->m_ProgressMsg).c_str(), -1, &msgFont, Gdiplus::PointF(2.0f, pInstance->m_pImage->GetHeight()-34.0f), &msgBrush );
				}
			}
